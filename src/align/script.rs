...
mod tests {
  ...
  fn han_with_kana_context_is_ja() {
    let ctx = SegmentContext { has_kana: true, has_hangul: false };
    assert_eq!(
      script_to_lang('語', ctx, None),
      CharClass::Lang(Lang::Ja),
    );
  ...
  fn han_with_hangul_context_is_ko() {
    let ctx = SegmentContext { has_kana: false, has_hangul: true };
    assert_eq!(
      script_to_lang('語', ctx, None),
      CharClass::Lang(Lang::Ko),
    );
  ...
  fn han_kana_beats_hangul_when_both_present() {
    let ctx = SegmentContext { has_kana: true, has_hangul: true };
    assert_eq!(
      script_to_lang('語', ctx, None),
      CharClass::Lang(Lang::Ja),
    );
  ...
  #[test]
  fn latin_with_zh_hint_falls_back_to_en() {
    // Zh is non-Latin; the hint must not bleed through.
    let ctx = SegmentContext::default();
    assert_eq!(
